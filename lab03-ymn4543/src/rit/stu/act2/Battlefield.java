...
public class Battlefield {
    ...
    /** the helicopter that will rescue hostages */
    ...
    private void battle() {
        ...
        while(enemyBase.getNumHostages()>0 && bunker.getNumSoldiers() >0){
            ...
            Hostage rescued = enemyBase.rescueHostage(soldier);
            if (rescued != null) {
                System.out.println(rescued.toString() + " rescued from enemy base by " + soldier.toString() + ".");
                int roll = Battlefield.nextInt(1, 100);
                System.out.println(soldier.toString() + " encounters the predator who rolls a " + roll);
                if (roll > predator.getChanceToBeatSoldier()) {
                    soldier.victory(predator);
                    predator.defeat(soldier);
                    chopper.boardPassenger(rescued);
                    bunker.fortifySoldiers(soldier);
                } else {
                    predator.victory(soldier);
                    soldier.defeat(predator);
                    roll = Battlefield.nextInt(1, 100);
                    System.out.println(rescued.toString() + " encounters the predator who rolls a " + roll);
                    if (roll <= predator.getChanceToBeatHostage()) {
                        predator.victory(rescued);
                        rescued.defeat(predator);
                    } else {
                        rescued.victory(predator);
                        predator.defeat(rescued);
                        chopper.boardPassenger(rescued);
                    }
                }
        ...
        while(bunker.getNumSoldiers()!=0) {
            Soldier s2 = bunker.deployNextSoldier();
            chopper.boardPassenger(s2);
        }
